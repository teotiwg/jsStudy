...
class UpdateForm extends Component { 
    ...
    /*
    각 input상자에서 입력한 내용이 있을때 state값을 변경하기
    위한 화살표 함수. setState의 name부분은 input의 name, 그리고 state의
    key와 동일한 값을 가진다. 
    */
    ...
    render(){
        ...
        <article>
            ...
            <form action="/create_process" method="post" onSubmit={function(e){
                ...
                {/* 기존 게시물에 대한 수정이므로 게시물 아이디를 저장할 hidden상자 필요함 */}
                {/*  
                App.js에서 기존 게시물을 읽어온 후 props를 통해 전달한 데이터를
                각 input상자에 삽입한다. 하지만 값을 수정하는 것은 불가능하다. 
                props는 읽기전용 이므로 값에 대한 수정을 하려면 state를 통해서만
                가능하다. 
                */}
                {/*  
                읽기전용인 props값은 변경할 수 없으므로 state값을 통해 입력값을
