	...
	$(document).ready(function(){
		...
		$('#goodsOptionId1').change(function(){
			$.ajax({
				...
				success:function(d){
					// hidden으로 생성된 기본가격을 가져와서 숫자로 변경
					...
					// 계산된 금액을 hidden박스에 입력하고
					$('#totalPrice').val(totalPrice);
					// 화면에 출력
					$('#priceDisplay').html("총삼품금액: "+totalPrice);
					// 옵션을 처음 선택햇다면 기존 문자열을 지우고 옵션을 추가
					if($('#totalPrice').html()=="여기에 선택한 옵션 리스팅됌"){
						$('#goodsList').empty();
					}
						$('#goodsList').append(d);
					},
